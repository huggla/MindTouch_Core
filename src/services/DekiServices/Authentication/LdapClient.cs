    ...
    public class LdapClient {
        ...
        private LdapConnection Bind(string hostname, string bindDN, string bindPW) {
            ...
            try {
                ...
                if (!conn.Bound) {
                    ...
                    throw new DreamAbortException(DreamMessage.AccessDenied("MindTouch Core LDAP Service", string.Format("An LDAP bind was not established. Server: '{0}'. BindingDN: '{1}'. Password provided? '{2}'", hostname, bindDN, string.IsNullOrEmpty(bindPW) ? "No." : "Yes.")));
                }
            } catch (LdapException x) {
                UnBind(conn);

                if (x.ResultCode == LdapException.INVALID_CREDENTIALS) {
                    throw new DreamAbortException(DreamMessage.AccessDenied("MindTouch Core LDAP Service", string.Format("Invalid LDAP credentials. Server: '{0}'. BindingDN: '{1}'. Password provided? '{2}'", hostname, bindDN, string.IsNullOrEmpty(bindPW) ? "No." : "Yes.")));
                } else {
                    throw;
                }
            }
        ...
        private string EscapeLdapString(string original) {
            ...
            foreach(char c in original) {
                ...
                case '>': // rfc 2253
                case ';': // rfc 2253
                case '(': // throws parsing error
                case ')': // throws parsing error
                case '/': // invalid pre-windows 2000 username char
                case '[': // invalid pre-windows 2000 username char
                case ']': // invalid pre-windows 2000 username char
                case ':': // invalid pre-windows 2000 username char
                case '|': // invalid pre-windows 2000 username char
                ...
                // convert to hex \xx
                case ' ': // throws parsing error; rfc 2253 (start and end of string) 
