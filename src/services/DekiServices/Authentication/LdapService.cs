...
namespace MindTouch.Deki.Services {
    ...
    [DreamService("MindTouch LDAP Authentication Service", "Copyright (c) 2006-2010 MindTouch Inc.",
       Info = "http://doc.opengarden.org/Deki_API/Reference/LdapAuthentication",
        SID = new string[] { 
            "sid://mindtouch.com/2007/05/ldap-authentication",
            "http://services.mindtouch.com/deki/stable/2007/05/ldap-authentication",
            "http://services.mindtouch.com/deki/draft/2006/12/ldapdirectory"
        }
    )]
    [DreamServiceConfig("hostname", "string", "Hostname or ip of domain controller or ldap server. By default port 389 is used with a plaintext connection and 636 is used with SSL enabled. A port can be specified with a :port suffix.")]
    ...
    [DreamServiceConfig("bindingdn", "string", "The DN to use for binding to LDAP. Use $1 to substitute with user name. Enter a DN of a specific user if also providing a bindingpw. ActiveDirectory example: $1@sd.mindtouch.com OpenLdap example: CN=$1,DC=sd,DC=mindtouch,DC=com")]
    ...
    [DreamServiceConfig("groupmembershipquery", "string?", "Use a custom query to return the groups a user belongs to where $1 is the username. Only use this if you're having issues returning groups that a user belongs to. OpenLDAP example: (&(uniqueMember=$1)(objectClass=groupOfUniqueNames))")]
    ...
    public class LdapAuthenticationService : DreamService {
        ...
        private LdapClient GetLdapClient(DreamContext context, DreamMessage request, bool requireAuth) {
            ...
            if (_config.VerboseLogging) {
                _log.DebugMethodCall(context.Feature.VerbSignature, string.Format("Performing LDAP lookup uri: '{0}' username: '{1}' pw: '{2}'", context.Feature.VerbSignature, authuser, authpassword));
            }
