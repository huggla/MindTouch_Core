 ...
 * @class
 */
CKEDITOR.themes.add( 'mindtouch', (function()
{
	...
	return /** @lends CKEDITOR.theme */ {
		build : function( editor, themePath )
		{
			...
			var container = CKEDITOR.dom.element.createFromHtml( [
				'<span' +
					...
					'<span class="' , CKEDITOR.env.cssClass, '" role="presentation">' +
						'<span class="cke_editor" role="presentation" style="display:block">' +
							'<span class="cke_', editor.lang.dir, '" role="presentation">' +
								'<span', topHtml ? '' : ' style="display:none"', ' id="cke_top_', name, '" class="cke_top" role="presentation">', topHtml, '</span>',
							'</span>' +
						'</span>' +
			...
			container.getChild( [1, 0] ).unselectable();
			container.getChild( [1, 1, 0, 0, 1] ).unselectable();
...
CKEDITOR.editor.prototype.lock = function()
{
	...
	mask.setStyle( 'width', container.$.offsetWidth + 'px' );
	mask.setStyle( 'height', container.$.offsetHeight + 'px' );
