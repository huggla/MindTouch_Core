...
#------------------------------------------------------------------------------
	...
	function open( $server, $user, $password, $dbName, $dbPort ) {
		...
		if ( !$success ) {
			...
			//execute these once a successful connection is opened; required for UTF-8 support
	...
	/**
	 * Fetch db result as associate array
	 *
	 * @param object $res
	 * @return array
	 */
	 ...
	 * SELECT wrapper
	 */
	function select( $table, $vars, $conds='', $fname = 'Database::select', $options = array(), $like = array() )
	{
		if( is_array( $vars ) ) {
			$vars = implode( ',', $vars );
		}
		...
		if (!empty($like)) {
			if ( is_array( $like ) ) {
				$like = $this->makeList( $like, LIST_LIKE );
			}
		}
		...
		if( !empty( $conds ) ) {
			...
			$conds = (!empty($like)) ? $conds . ' AND ' . $like : $conds;
			$sql = "SELECT $vars $from $useIndex WHERE $conds $tailOpts";
		} elseif ( !empty($like) ) {
			$sql = "SELECT $vars $from $useIndex WHERE $like $tailOpts";
		} else {
	 ...
	 * e.g. selectRow( "cur", array( "page_id" ), array( "page_namespace" => 0, "page_title" => "Astronomy" ) )
	 *   would return an object where $obj->page_id is the ID of the Astronomy article
	 *
	 * @todo migrate documentation to phpdocumentor format
	 */
	function selectRow( $table, $vars, $conds = '', $fname = 'Database::selectRow', $options = array(), $like = array(), $fetchArray = false ) {
		$options['LIMIT'] = 1;
		$res = $this->select( $table, $vars, $conds, $fname, $options, $like );
		if ( $res === false || !$this->numRows( $res ) ) {
			return false;
		}
		if ($fetchArray)
		{
			$obj = $this->fetchArray( $res );
		}
		else
		{
	 ...
	 * @param bool $fetchArray - set this true if you want to get associate array instead of object as db result
	 * @return unknown
	 */
	function selectRows( $table, $vars, $conds = '', $fname = 'Database::selectRows', $options = array(), $like = array(), $fetchArray = false ) {
		$res = $this->select( $table, $vars, $conds, $fname, $options, $like );
		//if ( $res === false || !$this->numRows( $res ) ) {
		if ( $res === false ) {
			return false;
		...
		for ($rowIndex = 0; $rowIndex < $this->numRows($res); $rowIndex++)
		{
		  if ($fetchArray)
		  {
		  	$obj[] = $this->fetchArray( $res );
	...
	function makeList( $a, $mode = LIST_COMMA ) {
		...
		foreach ( $a as $field => $value ) {
			if ( !$first ) {
				if ( $mode == LIST_AND || $mode == LIST_LIKE) {
					$list .= ' AND ';
				} else {
					$list .= ',';
				}
			...
			if ( $mode == LIST_AND && is_numeric( $field ) ) {
				$list .= "($value)";
			} else if ( $mode == LIST_AND && is_null($value) ) {
				$list .= "$field IS NULL";
			} else if ( $mode == LIST_AND && is_string($value) && $value == "NOT NULL" ) {
				$list .= "$field IS NOT NULL";
			} elseif ( $mode == LIST_AND && is_array ($value) ) {
				$list .= $field." IN (".$this->makeList($value).") ";
			} elseif ( $mode == LIST_LIKE ) {
				$list .= $field." LIKE '".$value."'";	
			} else {
	 ...
	 */
	function tableName( $name , $suppressQuotes = false) {
		
		//hackity hack hack convert 'user' table references to 'users'
		if ($name == 'user') {
			$name = 'users';
		}
		//hackity hack hack convert 'cur' table references to 'pages'
 		if ($name == 'cur') {
 			$name = 'pages';
 		}
		# Skip quoted literals
		if ( $name{0} != '`' and $name{0} != '(' and !stristr($name, ' ')) {
	 ...
	 * Get the column type
	 */
	function columnType ($table, $field) {		
		$table = $this->tableName( $table );
