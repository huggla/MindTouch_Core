...
class WebRequest {
	...
	/**
	 * Recursively trim strings in the given array.
	 *
	 * @param array $data string or array
	 * @return mixed trimed version of the given
	 * @private
	 */
	function trimData ( $data ) {
		if ( is_array($data) ) {
			foreach ( $data as $key => $val ) {
				$data[$key] = $this->trimData( $val );
			}
			return $data;
		} else {
			return trim( $data );
	...
	function getGPCVal( &$arr, $name, $default ) {
		...
		$name = str_replace(array('.', ' ', '['), array('_', '_', '_'), $name);
		...
		if( isset( $arr[$name] ) ) {
			...
			return $this->normalizeUnicode( $this->trimData( $data ) );
		} else {
			return $default;
		}
	}
	...
	/**
	 * Return the type of file uploaded
	 * @param string $key
	 * @return integer
	 *
	 * added by royk
